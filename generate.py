...
IMAGE_PATH = "./input_images/water.png"
# IMAGE_PATH = "./input_images/bus.png"
IMAGE_PATH = "./input_images/dog.png"
# IMAGE_PATH = "./input_images/cycle.png"
...
caption, full_ids, all_hidden_states, pixel_values, image_grid_thw, prompt_len = (
    generate_caption_and_all_hidden_states(
        ...
        prompt = "Describe the image briefly at most 10 words.",
        max_new_tokens=40,
    )
)
...
def overlay_heatmap(image, grid, alpha=0.2, show_grid=True, H_patches=None, W_patches=None):
    """
    Overlay similarity heatmap on image with optional patch grid lines.
    
    Args:
        image: PIL Image
        grid: Similarity grid [H_patches, W_patches]
        alpha: Heatmap transparency (0=invisible, 1=opaque)
        show_grid: Whether to draw patch boundary lines
        H_patches: Number of patch rows (required if show_grid=True)
        W_patches: Number of patch columns (required if show_grid=True)
        
    Returns:
        PIL Image with heatmap (and optionally grid) overlay
    """
    ...
    # -------------------------------------------------------------------------
    if show_grid and H_patches is not None and W_patches is not None:
        # Calculate pixel coordinates for each patch boundary
        patch_height = H / H_patches
        patch_width = W / W_patches
        
        # Draw horizontal lines
        for i in range(1, H_patches):
            y = int(i * patch_height)
            cv2.line(blended, (0, y), (W, y), color=(255, 255, 255), thickness=1)
        
        # Draw vertical lines
        for j in range(1, W_patches):
            x = int(j * patch_width)
            cv2.line(blended, (x, 0), (x, H), color=(255, 255, 255), thickness=1)
...
def build_frames(image, tokens, sim_vectors, H_patches, W_patches, out_dir="frames", alpha=0.5):
    """Generate frame images for animation with patch grid overlay."""
...
def generate_token_evolution_gifs(
    ...
    H_patches,
    W_patches,
    out_dir="output/token_evolution",
    image_name="image",
    alpha=0.5
):
    """
    Generate GIFs showing how attention evolves across layers for each token.
    
    CONCEPT:
    Instead of showing all tokens in one layer, this shows ONE token across
    ALL layers. This reveals how attention becomes refined as information
    flows through the network.
    
    OBSERVATIONS YOU MIGHT SEE:
    - Early layers (0-5): Diffuse, broad attention (low-level features)
    - Middle layers (6-15): Emerging focus (semantic grouping)
    - Late layers (16+): Sharp, targeted attention (high-level understanding)
    
    WORKFLOW:
    1. Compute similarities for ALL layers and ALL tokens
    2. Pivot the data: organize by token (instead of by layer)
    3. For each token, generate frames showing layer progression
    4. Create GIF directly in output folder (no subfolders)
    ...
    Args:
        ...
        prompt_len: Length of prompt portion
        image_embs: Image patch embeddings
        H_patches: Number of patch rows
        W_patches: Number of patch columns
        out_dir: Output directory for GIFs
        image_name: Prefix for output files
    ...
    # -------------------------------------------------------------------------
    # Structure: all_sims[layer_idx] = (sim_vectors, tokens)
    # where sim_vectors is a list of [num_patches] arrays
    
    print("Computing similarities for all layers...")
    all_sims = []
    tokens = None
    
    for layer_idx in range(num_layers):
        hidden_states_layer = all_hidden_states[layer_idx]
        sim_vectors, tok = compute_similarity(
            hidden_states_layer=hidden_states_layer,
            full_ids=full_ids,
            prompt_len=prompt_len,
            image_embs=image_embs,
        )
        all_sims.append(sim_vectors)
        
        # Tokens are the same across layers, just store once
        if tokens is None:
            tokens = tok
            num_tokens = len(tokens)
            print(f"  Found {num_tokens} tokens to analyze")
    
    # -------------------------------------------------------------------------
    # STEP 2: PIVOT DATA STRUCTURE
    # -------------------------------------------------------------------------
    # Reorganize from [layer][token] to [token][layer]
    # token_layer_sims[token_idx] = list of similarity vectors across layers
    
    print("\nReorganizing data by token...")
    token_layer_sims = []
    for token_idx in range(num_tokens):
        # Collect this token's similarity across all layers
        layer_progression = []
        for layer_idx in range(num_layers):
            sim_vec = all_sims[layer_idx][token_idx]  # [num_patches]
            layer_progression.append(sim_vec)
        token_layer_sims.append(layer_progression)
    
    print(f"  Organized {num_tokens} tokens × {num_layers} layers")
    ...
    # -------------------------------------------------------------------------
    # STEP 4: GENERATE GIF FOR EACH TOKEN
    # -------------------------------------------------------------------------
    print(f"\nGenerating evolution GIFs...")
    
    for token_idx in range(num_tokens):
        token_text = tokens[token_idx]
        layer_progression = token_layer_sims[token_idx]
        
        print(f"\n  Token {token_idx}/{num_tokens-1}: '{token_text}'")
        
        # Create temporary frames for this token
        temp_frames = []
        
        for layer_idx in range(num_layers):
            sim_vec = layer_progression[layer_idx]  # [num_patches]
            ...
            # Add caption showing token and layer info
            fig.text(
                0.5, 0.95,
                f"Token: '{token_text}'",
                fontsize=20,
            ...
            temp_frame = f"/tmp/temp_token{token_idx}_layer{layer_idx}.png"
            plt.savefig(temp_frame, dpi=120, pad_inches=0.2, bbox_inches='tight')
            plt.close(fig)
            
            temp_frames.append(temp_frame)
        
        # Create GIF directly in output folder
        # Sanitize token text for filename
        safe_token = token_text.replace("/", "_").replace("\\", "_").replace(" ", "_")
        safe_token = "".join(c for c in safe_token if c.isalnum() or c in "_-")
        
        gif_filename = f"token_{token_idx:02d}_{safe_token}_layer_evolution.gif"
        gif_path = os.path.join(out_dir, gif_filename)
        
        # Create GIF
        imgs = [imageio.imread(f) for f in temp_frames]
        imageio.mimsave(gif_path, imgs, duration=0.3)  # Faster playback
    ...
    print(f"\n{'='*70}")
    print(f"TOKEN EVOLUTION COMPLETE")
    print(f"All GIFs saved in: {out_dir}/")
    print(f"{'='*70}\n")
...
def create_patch_grid_reference(image, H_patches, W_patches, output_path="patch_grid_reference.png"):
    """
    Create a reference image showing the patch grid boundaries.
    
    This helps understand how the vision encoder divides the image into patches.
    Each patch is one "visual token" that the model processes.
    
    Args:
        image: PIL Image
        H_patches: Number of patch rows
        W_patches: Number of patch columns
        output_path: Where to save the reference image
        
    Returns:
        PIL Image with black grid lines overlaid
    """
    ...
    print(f"\nCreating patch grid reference:")
    print(f"  Image size: {W}×{H} pixels")
    print(f"  Patch grid: {H_patches}×{W_patches} = {H_patches * W_patches} patches")
    print(f"  Each patch: ~{patch_width:.1f}×{patch_height:.1f} pixels")
    
    # Draw horizontal lines (thicker for better visibility)
    for i in range(1, H_patches):
        y = int(i * patch_height)
        cv2.line(img, (0, y), (W, y), color=(0, 0, 0), thickness=2)
    
    # Draw vertical lines
    for j in range(1, W_patches):
        x = int(j * patch_width)
        cv2.line(img, (x, 0), (x, H), color=(0, 0, 0), thickness=2)
    
    # Convert back to PIL and save
    grid_img = Image.fromarray(img)
...
# Create patch grid reference image
patch_grid_img = create_patch_grid_reference(
    raw_image, 
    H_patches, 
    W_patches, 
    output_path="patch_grid_reference.png"
)

patch_grid_img  # Display in notebook
...
# Generate layer-by-layer visualizations (all tokens in each layer)
# Extract just the filename without path or extension for image_name
...
# Generate token evolution visualizations (each token across all layers)
